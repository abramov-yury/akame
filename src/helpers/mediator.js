export const MEDIATOR = {

  showNavigation () {

    throw new Error("Mediator method not implemented: SHOW_NAVIGATION");

  },

  showMenu () {

    throw new Error("Mediator method not implemented: SHOW_MENU");

  },

  clearContent () {

    throw new Error("Mediator method not implemented: CLEAR_CONTENT");

  },

  mountIntroController () {

    throw new Error("Mediator method not implemented: MOUNT_INTRO_CONTROLLER");

  },

  mountGalleryController () {

    throw new Error("Mediator method not implemented: MOUNT_GALLERY_CONTROLLER");

  },

  mountAboutController () {

    throw new Error("Mediator method not implemented: MOUNT_ABOUT_CONTROLLER");

  },

  mountBlogController () {

    throw new Error("Mediator method not implemented: MOUNT_BLOG_CONTROLLER");

  },

  createPopup () {

    throw new Error("Mediator method not implemented: CREATE_POPUP");

  },

  getCurrentPhotos () {

    throw new Error("Mediator method not implemented: GET_CURRENT_PHOTOS");

  },

};
